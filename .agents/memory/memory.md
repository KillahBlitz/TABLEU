...
20. **[2026-08-21] Real-time Sitemap Sync — Socket.io:**
    - **Backend:** Installed `socket.io`. Added `backend/src/socket.js` (`setIO`/`getIO` holder to avoid circular imports). Modified `server.js` to use `createServer(app)` + `new Server(httpServer)`, with `sitemap-room` group. Emits `sitemap:updated` (nodes/edges/library/viewport) from `sitemapController.js` after every save and clear. Broadcasts `sitemap:cursor` to all other clients; emits `sitemap:cursor:leave` on disconnect.
    - **Frontend:** Installed `socket.io-client`. Created `services/sitemapSocket.js` (connect with JWT auth, reconnect logic). Added Vite proxy for `/socket.io` with `ws: true`. Modified `SitemapView.jsx`: connects socket on mount, disconnects on unmount; receives `sitemap:updated` and updates nodes/edges/library ONLY if `saveStatus === 'saved'` (avoids overwriting in-progress edits); throttled cursor emit (50ms, canvas coords). Modified `SitemapCanvas.jsx`: new `remoteCursors` prop + `.sitemap-cursors-layer` renders `<MousePointer2>` + name label in transform-plane canvas space for each remote user.
    - **Roles:** Admins see CRUD controls; all roles see the same live-synced sitemap. Viewport (pan/zoom) is NOT synced — each user moves independently.
...
20. **[2026-08-21] Galería de Pantallas (Drawer) + Instant Base64 Data URLs + Note Color Picker Fix:**
    - **Note Color Picker:** Fixed clipping bug where `.sitemap-color-popover` was cropped by `overflow: hidden` on `.sitemap-node-card`. Repositioned to `.sitemap-note-color-popover` (`top: 36px; right: 8px; z-index: 70`) and applied explicit `style={{ backgroundColor: node.color }}` on card container.
    - **Instant Base64 Rendering:** Re-architected image ingestion to convert any uploaded, pasted, or dropped file into Base64 Data URLs via `FileReader.readAsDataURL` on client side. Guaranteed 100% instant rendering in any browser/proxy configuration with zero network dependency.
    - **Galería de Pantallas (`SitemapLibraryDrawer.jsx`):** Added a sliding drawer panel accessible via "Galería / Subir" button in toolbar. Supports uploading multiple screen images, shows miniature cards with previews and titles, enables dragging screens directly onto canvas coordinates, or clicking "+ Agregar" to place at viewport center.
    - **Persistent Library Storage:** Added `library` schema array to `Sitemap.js` and `sitemapController.js` to persist uploaded screen assets in MongoDB.
