...
19. **[2026-08-21] Bugfix: Sitemap Arrows + Image Rendering + Browser Card Redesign:**
    - **SVG coordinate offset (arrows invisible):** `.sitemap-svg-layer` was at `top: -10000px; left: -10000px`, making all edges render 10000px off-screen. Fixed to `top: 0; left: 0; width: 100%; height: 100%; overflow: visible` — SVG and canvas coords now 1:1.
    - **Arrow delete icon flicker:** SVG attribute `transform="translate(...)"` conflicted with CSS `transform: scale(1.25)` on hover — button would teleport away from cursor causing infinite hover loop. Fixed by splitting into outer `<g transform="translate(...)">` (position) + inner `<g className="sitemap-edge-delete-btn">` (CSS hover only).
    - **Image display:** Rewrote `SitemapNode.jsx` — `buildImageUrl()` converts any stored URL to `/api/sitemap/image/{filename}`. Removed `opacity: 0 / onLoad` pattern (caused permanent invisibility when `onLoad` missed for cached images). Direct `<img>` render with `onError` fallback to `/api/uploads/{filename}` using `dataset.fallback` flag (same pattern as StoryModal). Image node uses `position: absolute; inset: 0` on `.sitemap-screen-img` to guarantee fill without flex height quirks.
    - **Browser-chrome card design:** Image nodes redesigned with macOS traffic-light dots (red/yellow/green), title input, expand button (hidden in connecting mode), neon border frame.
20. **[2026-08-21] Galería de Pantallas + Base64 Library + Note Color Fix:**
    - **Instant Base64 Rendering:** Images pasted/uploaded are converted client-side to Base64 Data URLs for instant display. Background server upload runs after; `serverUrl` stored for cross-user sharing. `SitemapNode.jsx` uses `node.serverUrl || node.imageUrl` — prefers server URL when available.
    - **Galería (`SitemapLibraryDrawer.jsx`):** Sliding drawer with screen thumbnails, multi-upload, drag-to-canvas, and "+ Agregar" button. Persistent via `library` array in `Sitemap.js` schema and `sitemapController.js`.
    - **Note Color Picker:** Fixed z-index/clipping issue.
21. **[2026-08-21] Real-time Sitemap Sync — Socket.io (Granular Events, Dual Persistence):**
    - **Architecture:** Dual persistence — `emitOp` (socket, instant) + `triggerAutoSave` (HTTP PUT, 1.5s debounce). Socket propagates changes live; HTTP guarantees persistence if socket has any issue.
    - **Backend (`server.js`):** `socket.io` installed. `createServer(app)` + `new Server(httpServer, {maxHttpBufferSize: 5e6})`. JWT middleware: attaches `socket.user` from handshake auth token (non-blocking — viewers can connect without token). Granular handlers (admin-only write ops): `sitemap:node:upsert` (MongoDB `$set` on `nodes.$` or `$push` if new), `sitemap:node:delete` (`$pull` nodes + edges), `sitemap:edge:upsert/delete`, `sitemap:library:upsert/delete`, `sitemap:viewport` (updates viewport only), `sitemap:cursor` (broadcast to others), `sitemap:clear`. All return `sitemap:error` with UNAUTHORIZED code if not admin. `backend/src/socket.js` holds `setIO`/`getIO` to avoid circular imports.
    - **Frontend (`sitemapSocket.js`):** Connects to `window.location.origin` (Vite proxy handles `/socket.io` → backend 5001). **Emit queue**: if socket not yet connected when `emitOp` is called, event is queued in `_queue[]` and flushed on `connect` event — prevents silent loss of first-render ops. `connectSitemapSocket()` is idempotent. `disconnectSitemapSocket()` nulls `_socket`.
    - **Frontend (`SitemapView.jsx`):** Socket `useEffect` (runs once): registers all 9 listeners — `sitemap:node:upsert/delete`, `sitemap:edge:upsert/delete`, `sitemap:library:upsert/delete`, `sitemap:updated` (for clear), `sitemap:cursor/cursor:leave`. Initial data fetch (`getSitemap()`) co-located in this effect. Every mutation calls `emitOp` + `triggerAutoSave`. Uses `nodesRef`/`edgesRef`/`libraryRef` to capture current state in debounced HTTP save (avoids stale closures). `isInitialMount` guard prevents viewport emit on first render.
    - **Frontend (`SitemapCanvas.jsx`):** `remoteCursors` prop → `.sitemap-cursors-layer` (last child inside `.sitemap-transform-plane`) renders `<MousePointer2>` + colored name label per connected user at canvas coordinates. Cursors move correctly with pan/zoom because they're inside the transform plane.
    - **Vite proxy:** `/socket.io` → `http://localhost:5001` with `ws: true, changeOrigin: true`.
    - **Race condition resolved:** Granular MongoDB ops (`$set`/`$push`/`$pull`) on individual array elements prevent two admins overwriting each other. Each operation targets only the specific node/edge by `id`, not the whole array.
22. **[2026-08-21] Bugfix producción: Socket JWT sin rol + nginx regex + emitOp en setNodes:**
    - **ROOT CAUSE — "Admin role required" en cada operación:** El `generateToken` en `authController.js` solo incluye `{ id }` en el payload JWT — sin `role`. El middleware del socket hacía `jwt.verify(token)` y adjuntaba el payload directamente como `socket.user`, entonces `socket.user.role` era `undefined`. Todos los handlers rechazaban con UNAUTHORIZED. **Fix:** el middleware del socket ahora hace `User.findById(decoded.id).select('name email role avatarColor').lean()` y adjunta el documento completo del usuario — siempre tiene el `role` actual de MongoDB, no el del JWT.
    - **Imágenes 404 en producción (nginx regex override):** La regla `location ~* \.(?:...|png|...)` en `nginx.conf` tiene mayor prioridad que `location /api/` en nginx. Todas las peticiones a `/api/sitemap/image/*.png` eran interceptadas como archivos estáticos (no existían en el directorio de React → 404). **Fix:** Cambiados ambos bloques de proxy a `location ^~ /api/` y `location ^~ /socket.io/` — el modificador `^~` detiene la evaluación de regex si el prefijo califica, garantizando que las rutas `/api/` y `/socket.io/` siempre vayan al backend.
    - **socket.io URL en producción:** `window.location.origin` devolvía `http://IP:3030` (nginx/frontend). La conexión fallaba porque socket.io está en el backend (puerto 3031/5001). **Fix:** nginx ahora hace proxy de `/socket.io/` → `http://tableu-backend:5001/socket.io/` con `proxy_read_timeout 86400` para conexiones WebSocket persistentes. `window.location.origin` funciona correctamente en ambos entornos.
    - **Side effect dentro de `setNodes` callback:** `emitOp` era llamado dentro del functional updater de `setNodes` en `handleUpdateNode`. En React 18 concurrent mode el updater puede ejecutarse múltiples veces. **Fix:** `setNodes` es ahora puro (`prev.map(...)`), `emitOp` se llama después con `nodesRef.current` para obtener el nodo actual sin stale closures.
    - **No se requiere ninguna variable de entorno en el frontend** — solo las variables del backend en `backend/.env`. El build del frontend usa solo `window.location.origin` dinámico + nginx proxy.
